...
void Displ_Select(void) {
	HAL_GPIO_WritePin(SPI5_CS_GPIO_Port, SPI5_CS_Pin, GPIO_PIN_RESET);	// select display
}
...
void Displ_Transmit(GPIO_PinState DC_Status, uint8_t* data, uint16_t dataSize, uint8_t isTouchGFXBuffer ){
	...
	Displ_Select();
	HAL_GPIO_WritePin(GPIOB, LCD_DC_Pin, DC_Status);
...
void Displ_Init(Displ_Orientat_e orientation){
	HAL_GPIO_WritePin(SPI5_CS_GPIO_Port, SPI5_CS_Pin, GPIO_PIN_SET); 		// unselect display (will be selected at writing time)
	ILI9XXX_Init();
...
#ifndef DISPLAY_DIMMING_MODE
	return HAL_GPIO_ReadPin(GPIOB, LCD_BL_Pin);
#else
...
void HAL_TIM_PeriodElapsedCallback (TIM_HandleTypeDef * htim){
	if (htim==&TGFX_T){
		  touchgfxSignalVSync();
		HAL_GPIO_TogglePin(TIMER_TEST_GPIO_Port, TIMER_TEST_Pin);
	}
}
#endif //DISPLAY_USING_TOUCHGFX
